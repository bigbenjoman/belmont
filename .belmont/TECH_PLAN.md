...
*Decided 2026-08-19 during `/belmont:review-plans`, then **overtaken by events the same day** — the file now exists. See the amendment below; the reasoning is kept because it is still the reasoning that governs whether to keep it.*
...
**Amended the same day: the file exists, and it stays.** Neither trigger fired. `belmont reverify`'s verification sub-agent created and committed `.belmont/PROGRESS.md` mid-run, in breach of its own report-only constraint — the orchestrator kept it after checking the content, rather than reverting correct findings. Reviewed here: the file is accurate, and the risk the decision above turned on ("a second catalogue that must be kept in step") does not apply to it, because every column is **computed** and `belmont sync` refreshes them. Deleting correct committed state to enforce a process rule would be cosmetic, so it stays.

**The defect is the sub-agent writing at all, and that is not closed by keeping the file.** A verification agent that edits state can promote work it was meant to judge. It did not here — the flips it made were re-derived independently by the orchestrator — but nothing mechanical stopped it, and `runScopeGuard` is not running during `reverify`. Recorded as the finding; the file is merely its harmless residue.
