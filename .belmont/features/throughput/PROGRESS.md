...
- [x] P0-M1-FIX-8: Reconcile the three documentation inconsistencies the fix batch left behind — all one root cause, the "one wrong sentence, N copies" pattern this batch documented and then reproduced. (a) `NOTES.md:21` and `MILESTONE-M1.done.md:460` still carry the claim `P0-M1-FIX-3` retracted, that `origin/feat/maintenance-ci` is "29/31 landed with both leftovers verified present" — NOTES.md is loaded into every session, so the retracted claim is the one an agent actually reads; annotate the archive and point the NOTES bullet at `docs/branch-triage.md` as canonical, matching how FIX-2 and FIX-6 handled theirs. (b) `docs/branch-triage.md:98-107` says "the rows below it were not re-audited to that bar" — false at HEAD, because FIX-5 re-audited the immediately following row; name the two rows that were (`worktree-git-excludes`, `wave-merge-state-loss`) and add FIX-5's missing §Revisions entry. (c) `CENSUS.md` §Why the estimate was wrong asserts "indentation is the upper bound on what extraction could move", then gives `feat-070` 140,957 B indented against 141,001 B moved — `censusFeature` counts blank lines inside a task body, which are not indented lines. Conclusions unaffected, but this is the canonical document feeding a human-gated decision.
