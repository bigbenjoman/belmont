...
**Build accessibility into the construction, not the audit.** Choose the floor first, then construct the ramp so it cannot fail it — space the steps so that a stated distance between them *guarantees* the ratio, and assign every role and its `on-` pair at that distance. A ramp built this way survives a change of anchor hue and a theme flip; a per-pair audit survives neither. Verify the pairs numerically afterwards as a check, never as the method. The reference gives the rule.

**Offer complete directions, not sliders — and generate them independently.** Present two or three *internally coherent* directions, each with a name, a one-line rationale tied to the product, its palette with semantic roles assigned (action, danger, success, neutrals — a palette without semantics is decoration), its type family and ramp, and its radius and elevation character. Draw each as a **real screen from this feature**, not swatch soup: the same screen, two or three times, is the only comparison that means anything.

Generate them **in parallel, one sub-agent per direction where your tool has them**, each with its own brief. Asking a single prompt for three options returns three shades of one idea — that is a measured result rather than a worry, and the same work found the combined prompt produced the *least* diverse output of the strategies tested. Then hold them to two rules: each direction must be **structurally incompatible** with the others — if one's header would sit happily on another's body, they are one direction wearing two palettes — and when the user chooses, **kill rather than blend**, because taking the safe half of each is averaging, which is the exact failure being avoided.

This is a mechanism-2 question in its purest form — publish the page, hand over the link, then ask.

**Typography, conservatively, and check rather than guess.** Prefer one family with a full weight range over a display pairing: harder to get wrong, ages better, and it is what most good products actually do. Where you propose a pairing, justify it by role rather than by mood. Two gates before any family reaches the page — an **overuse** check and a **licence** check, both single HTTP requests, both in the reference. They matter because a face the reviewer cannot legally see is not a candidate, and because the most-recommended "free" families in this space are not licensed the way everyone assumes.

**Read `references/brand-foundation-set.md` before you propose anything.** It carries the eight sections a brand foundation must contain and their order, the two gates, the candidate families, the palette construction rule, and the current tells. Read it only on this branch; it is worth nothing to a feature with a contract.

**Avoid the tells — but know that the list moves.** The recognisable look of machine-made design is real and its reputation is deserved, and it has already shifted once: the aesthetic the first generation of anti-slop advice recommended as the cure became the next generation's tell. So the reference dates its list, and no item on it is banned outright — each is legitimate where the product's own evidence leads there, and then you say so on the page. What is banned is arriving at one **by default**. Treat the enumeration as weighted evidence rather than a blocklist: a rule that rejects legitimate work is worse than no rule.
