# Tech Plan: Status badge

Plain files at the repo root — `badge.js` (CommonJS) and `badge.css`. No build
step, no dependencies. `badge.js` is required directly by `node --test`.
...
Typography — ratio 1.25, sizes 12px / 14px / 16px, line-height 1.4 body. Max 4 sizes.
