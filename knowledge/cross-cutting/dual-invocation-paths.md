...
- **Auto mode** — the Go CLI assembles the prompt itself and shells out to the AI tool's headless print mode (`claude -p`, `codex exec`, `pi -p`, …). Stdout is parsed for JSON decisions and structured events. **What gets injected is per-tool, and the blanket claim that discovery is bypassed is wrong for most of them:**
  - **claude, codex, gemini, copilot, cursor (5 of 7)** — `buildLoopPrompt` (`auto_loop.go`) emits a **bare slash reference** (`/belmont:implement --feature x`) plus milestone-scope prose, and `executeLoopAction` prepends steering text. That is *all* Belmont injects. The tool's own skill discovery resolves the reference, and the agent reads the skill body, agent files and project state from disk itself. Skill discovery is **not** bypassed here.
  - **pi, opencode (2 of 7)** — `adaptPromptForTool` (`toolexec.go`) deliberately rewrites the slash reference into "Read `.agents/skills/belmont/<skill>/SKILL.md` fully and follow it", because these tools do not resolve Belmont's slash form. For these two, discovery *is* bypassed — on purpose.
  - **windsurf** — has **no auto path at all**. `runAutoCmd` rejects it (`autocmd.go`) and `toolHeadlessArgs` has no case for it. The **install** surface is 8 tools; the **auto** surface is 7. `docs/supported-tools.md`'s headless table is the correct reference and already lists seven rows.

  This matters beyond pedantry: because the 5-tool majority runs inside the tool's own skill runtime, a skill's `references/` files, its `Skill` tool availability, and any user-scope skills it consults are all live under `claude -p` — which is why the design-authority ladder works headlessly on Claude.
...
- 2026-08-08 — corrected the auto-mode description. It claimed skill discovery is bypassed and Belmont injects the skill body, agent files and project state; that is true only for **pi and opencode**. For claude/codex/gemini/copilot/cursor Belmont injects a bare slash reference plus scope and steering prose, and the tool resolves the skill itself. Also recorded that windsurf has no auto path (install surface 8, auto surface 7). Found while landing the Design Contract work, which needed to know whether a skill's `references/` and user-scope design skills are reachable under `claude -p` — they are, precisely because discovery is not bypassed there.
