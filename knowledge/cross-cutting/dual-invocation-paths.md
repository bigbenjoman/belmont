...
- `skills/belmont/_partials/dispatch-strategy.md` already routes Claude (parallel sub-agents via the `Agent` tool, or `Task` on CLIs predating the rename) vs everyone else (sequential inline) — this is the dispatch half of the dual-path concern. It checks **by name**, which is exactly why #45 hit both paths at once: the same generated `SKILL.md` is read by an interactive `/belmont:implement` and inlined into the prompt `belmont auto` shells out with, so a wrong tool name disabled dispatch everywhere simultaneously and neither path could reveal the other's failure.
...
- 2026-08-14 — #45 is the cleanest example this entry has of a single prose defect hitting both paths at once: `dispatch-strategy.md`'s availability check named `Task`/`TeamCreate`, which the runtime stopped publishing, so every orchestrator skill fell back to inline execution in auto mode *and* in the REPL. Only a Tier 2 transcript could show it — no offline test reads a `SKILL.md`.
