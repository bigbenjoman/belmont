...
This is a real defect, separate from the exclude bug and with its own risk profile. It was left alone deliberately when `writeWorktreeGitExcludes` was removed, because the two fixes pull in opposite directions and bundling them would obscure both. Fixing it means either calling `syncFeatureStateAfterMerge` from `recoverMerge` or reworking the isolation so tracked edits are recoverable — decide which before touching either.
