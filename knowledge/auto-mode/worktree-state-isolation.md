...
- **The two merge paths agree about placement as well as markers.** #46 made them agree on markers and `TestBothMergePathsAgreeOnEveryMarkerPair` pins it; `TestBothMergePathsPlaceACarriedTaskTheSameWay` is the placement counterpart, and it also asserts both actually nested the task — two paths agreeing on the *wrong* placement would otherwise pass. Which function runs depends only on whether git registered a conflict, so a disagreement means the same wave produces a different document for reasons no user can see or predict.
