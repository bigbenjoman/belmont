...
- **Never point a live-run diagnostic at `belmont recover`.** These warnings can only fire while `auto.json` is active, and `listPreservedWorktrees` scans `worktreeBasePath(root)` — the exact directory `runAutoParallel` creates the running wave's worktrees in — with no active-run filter. So `belmont recover --list` mid-run reports the live worktrees as preserved and offers `--clean-all` on them. The messages name the unreadable path instead. (The missing filter in `recover` itself is a separate, unfixed hazard.)
