...
- **Carry a Tier 2 result across a later edit of the prose it measured.** A run licenses the exact wording it ran against, and nothing else. This is easy to do by accident and hard to spot afterwards: the run happens, the review lands, the prose is revised *in response to the review*, and the original numbers stay in the commit message and the knowledge entry — now describing an artifact that no longer exists. It happened on #45's second half, where the revision deliberately removed the paragraph's two most forceful sentences as overclaims; if rhetorical force was part of what moved the behaviour, the carried-over numbers were measuring the wrong thing. When you revise prose after a run, either re-run or say plainly which revision the numbers describe.
...
**That announcement paid for itself on the very next run.** With the tool name corrected, the same fixture began failing the *cross-run* comparison rather than any assertion: `failing-acceptance` filed two follow-up tasks for the seeded defect in one run and one in another, so an agent-invented ID (`P1-M1-FIX-2`) was present in run 1 and absent in run 2. The cause was legible from the transcripts: on `main` all three runs ran the agents inline, while on the fix branch they split between dispatching and not, and the two paths do not produce the same number of follow-ups. (Credit ordinary transcript reading for the `main`-side half — `main` carries no instruction to name an approach; that requirement arrived with #45. Mind the renumbering too: `main`'s C and the branch's B are both the inline path.) See [`../cross-cutting/dispatch-authorization.md`](../cross-cutting/dispatch-authorization.md) for what the split turned out to be. The nondeterminism was **uncovered, not introduced**: the old check could never pass, so uniform breakage had been masquerading as determinism. Two lessons follow for this harness. First, **a green cross-run check is not evidence of a working path** — it is equally consistent with a path that is uniformly broken, which is exactly what it was certifying. Second, the check iterates run 1's task IDs (`for id, v := range results[0]`), so it compares every ID **that run 1 produced** — an ID appearing only in a later run is never compared, making it one-directional. That still implicitly asserts that agent-invented follow-up IDs are stable across runs, which is much closer to asserting on prose than the seeded-task comparison is. **Later evidence settled it**: with the dispatch path made uniform (3 runs out of 3 dispatching), a batch still failed on `P1-M1-FIX-3` because one run filed three follow-ups for the seeded defect and another filed two, folding "add the npm test script" into the test task rather than splitting it out. Both verdicts were correct; only the task *granularity* differed. So the check is pinning a judgement call, and it will keep failing intermittently no matter what the dispatch prose says. If it is relaxed, scope it to the fixture's **seeded** IDs — never relax it merely because it is the thing currently failing.
...
- 2026-08-15 — recorded the follow-on: the #45 announcement made a cross-run failure diagnosable, and the failure showed that a green cross-run check can certify a uniformly broken path. Flagged that the check pins agent-invented follow-up IDs, and how to scope it if it is ever relaxed.
- 2026-08-15 — the check was then scoped to seeded IDs (`crossRunDivergences`, guarded by `TestCrossRunDivergencesScope`) after it failed with the dispatch path uniform, proving it pinned task granularity rather than behaviour. Added the "don't carry a Tier 2 result across a later edit of the prose it measured" failure mode, which this PR committed and had to correct.
