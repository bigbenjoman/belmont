...
**That announcement paid for itself on the very next run.** With the tool name corrected, the same fixture began failing the *cross-run* comparison rather than any assertion: `failing-acceptance` filed two follow-up tasks for the seeded defect in one run and one in another, so an agent-invented ID (`P1-M1-FIX-2`) was present in run 1 and absent in run 2. The cause was legible only because every child now names its approach out loud — on `main` all three runs said Approach C, while on the fix branch they split between A and B, and the two paths do not produce the same number of follow-ups. The nondeterminism was **uncovered, not introduced**: the old check could never pass, so uniform breakage had been masquerading as determinism. Two lessons follow for this harness. First, **a green cross-run check is not evidence of a working path** — it is equally consistent with a path that is uniformly broken, which is exactly what it was certifying. Second, the check compares *every* task ID, which implicitly asserts that agent-invented follow-up IDs are stable across runs — much closer to asserting on prose than the seeded-task comparison is. If it is ever relaxed, relax it by scoping it to seeded IDs, and never merely because it is the thing currently failing.
