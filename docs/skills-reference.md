...
- **A blocked task does not stop the loop.** `[!]` is a question queued for a person, so the loop moves past it to the next milestone whose dependencies are satisfied, and stops only when every remaining pending task is `[!]`. It never clears, answers, or withdraws one. On stop it reports the queue with `belmont blockers --feature <feature> --summary`.
- Stop conditions are counted, not judged: three consecutive phase failures, the same milestone failing verification twice (escalates to `/belmont:debug-manual`), or no state change across two iterations. Counts are written to `NOTES.md` under `## Loop decisions`, not held in context, so they survive compaction. Every stop condition lives inside the fenced recipe handed to `/loop` — that is the only text guaranteed to survive compaction.
