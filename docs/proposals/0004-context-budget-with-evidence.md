...
The tier axis is a barbell, not a gradient: **`high` or `low`, never `medium`.** Haiku is genuinely good for basic mechanical work — scanning, extraction, grep-and-read — and choosing it there is not a compromise. Sonnet is the false-economy tier on Claude: a premium over Haiku for output well short of Opus. A future PR proposing `low` for a demonstrably mechanical agent is arguable on its own evidence; one proposing `medium` is not.
