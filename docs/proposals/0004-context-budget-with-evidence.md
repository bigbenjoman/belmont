...
**Shared file with 0003 — the predicted conflict does not occur.** This PR's conditional-archive edits land at `_src/verify.md:110`/`:147`; 0003 touches `:70` and nearby. v3 and v4 claimed these were "four lines apart, inside default diff context, so they will conflict." **That was tested and is false.** On the real file at git 2.50.1: this PR rewriting `:110` plus a three-line insertion before `:114` rebases cleanly, both edits present, no markers — and stays clean with this PR's edit widened to `:108-112`. A conflict needs both sides to touch `:113`, i.e. zero unchanged lines between the changed regions.

The reasoning was wrong in kind. "Default diff context" governs `git am`/`git apply` applying a *patch file*; `git rebase` and `git merge` use the three-way content merge, which does not consult context lines. The line numbers were right; the inference from them was not.

Real overlaps with 0003 remain: both edit `_src/next.md` and `_src/references/models-yaml-format.md`. Under the current order **this PR lands first**, so 0003 rebases onto it.
