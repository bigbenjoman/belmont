...
- **`scripts/declsum/main.go` is built and verified** — branch `bigbenjoman:feat/declsum` (`901a5a3`). Cherry-pick it; do not rewrite it. Passed three controls on the real `cmd/belmont`: a verbatim four-function move (identical), the same move with two predicates inverted (caught), and a statement reorder (caught). Deterministic; adds no staticcheck findings.
  - **0005 §2.1 is stale in three ways** and should be corrected in the same pass: the baseline is **444** declarations, not 398 (the tool hashes `var`/`const` groups per-spec, so the figures are not comparable — do not treat 398 as a target); §2.1 does not say that **output normalisation is required**, without which the printer's position-derived line breaks make every legitimate move read as an edit; and it does not mention the third control.
- Decl-set diff empty at every extraction commit.

**Fold in `bigbenjoman:fix/worktree-git-excludes`** (`db25423`; build, vet, test, gofmt green; builds on all five targets). Briefly PR #22, closed in favour of landing here. Deletes `writeWorktreeGitExcludes` — a function git never read from, which staticcheck cannot flag because it *was* called — and brings two tests plus `knowledge/auto-mode/worktree-state-isolation.md`. See 0005 §4.2. Not a pure deletion, so the declsum expectation at the lint commit widens by two entries; 0005 §8 states the set.
...
- Two purity-proof mechanisms were specified before being run, and both were impossible. The third (0005 **§2.1**) has now been **built and run** — `feat/declsum`, three controls, all passing. The lesson held to the end: the spec's quoted 398-declaration baseline did not reproduce (it is 444), and the naive implementation fails the legitimate-move control outright until output is normalised. A mechanism described as verified is still a claim until you run it yourself.
