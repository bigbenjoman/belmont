...
/belmont:ux-design          Design authority (contract, screens, flows)
...
**Planning skills and structured questions**: Belmont's planning skills (`$belmont:product-plan`, `$belmont:ux-design` and `$belmont:tech-plan`) rely on Codex's structured question UI so users can answer keyboard-navigable pick-lists — for `ux-design` that includes the Design Contract approval itself. In Codex, run planning skills through plan mode:
...
- **State**: `.belmont/PR_FAQ.md`, `.belmont/PRD.md`, `.belmont/PROGRESS.md`, `.belmont/UX_DESIGN.md`, `.belmont/TECH_PLAN.md`, `.belmont/features/`
