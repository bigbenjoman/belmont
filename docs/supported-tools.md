...
**Model tiers**: Codex model IDs are single model slugs. Belmont's built-in tier mapping is optimized for the default quality/speed/cost balance: `low` uses `gpt-5.4-mini`, `medium` uses `gpt-5.3-codex`, and `high` uses `gpt-5.5`. Override any tier in `~/.belmont/local-llms.json` (or per-project `.belmont/local-llms.json`):
...
Per-shot env-var overrides: `BELMONT_CODEX_MODEL_<TIER>` (per tier) or `BELMONT_CODEX_MODEL` (all tiers). This is useful when OpenAI ships a newer Codex-available model before Belmont updates its built-in registry, or when you want a cheaper/faster tier mapping for a specific repository.
