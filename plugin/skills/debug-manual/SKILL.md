...
| `{base}/TECH_PLAN.md` | Decision narrative, library choices, file-path references, API shapes — anything where reality has moved past the written record. **Except a legacy `## Design Contract`** — features planned before `/belmont:ux-design` existed keep their design authority here, and it carries an approval stamp this flow cannot renew any more than it can renew `UX_DESIGN.md`'s |
...
- **No edits to `{base}/UX_DESIGN.md` or `.belmont/UX_DESIGN.md`**, or to the `design-preview.html` / `ux-flows.html` artifacts beside them. The design authority is written only by `/belmont:ux-design` and carries an `**Approval**` stamp this flow cannot renew. If the fix proved it wrong, surface that to the user and point them at `/belmont:ux-design --feature <slug>`. **The same prohibition follows the contract wherever it lives**: on a feature planned before the split, it is still a `## Design Contract` section inside `{base}/TECH_PLAN.md` and is equally off-limits.
