...
**Before you finish, read your own microcopy back against the contract's Microcopy Rules.** You wrote both; they drift apart anyway. Check the specific traps: a control or heading that names the thing as something it is not yet (a *booking* that is still a request), an apology where the rules forbid one — or, just as wrong, no apology on the one failure the rules say is yours, a promise the product cannot keep, and a hardcoded day or date inside a string that will be read on some other day.
