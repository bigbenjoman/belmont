...
- **Nothing invented may appear in either file.** No fabricated metrics, testimonials, logos, avatars or user names; no fake browser chrome or device frames; no screenshot of a product state that does not exist. These are review artifacts a human judges the design against, and a plausible invention is worse than a blank — it gets approved as evidence. Where a real string is not yet fixed, use the PRD's own copy, or a visibly neutral placeholder that could not be mistaken for content.
