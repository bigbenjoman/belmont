...
- **Auto mode** — the Go CLI shells out to the AI tool's headless print mode (`claude -p`, `codex exec`, `pi -p`, …) and parses its stdout. **What Belmont injects varies by tool, and the difference matters.** For five of the seven auto-capable CLIs (claude, codex, gemini, copilot, cursor) `buildLoopPrompt` emits a *bare slash reference* like `/belmont:implement --feature x` plus milestone-scope prose and steering text — the tool's own skill discovery still resolves the skill, and the agent reads the skill body, agent files and project state from disk itself. For **Pi and opencode only**, `adaptPromptForTool` deliberately rewrites that reference into a read-the-file instruction, so discovery *is* bypassed there, on purpose. Windsurf has no auto path at all — `belmont auto --tool windsurf` is rejected outright, so the **install** surface is eight tools while the **auto** surface is seven. Never conflate the two counts.
...
- **Design authority is derived once, in `/belmont:ux-design`, and only that skill writes it.** `ux-design` is a planning phase of its own, between `product-plan` and `tech-plan`. A feature with a UI and no Figma gets a `## Design Contract` in `{base}/UX_DESIGN.md`, approved interactively, alongside two self-contained artifacts (`design-preview.html`, `ux-flows.html`). `**Mode**` is the only machine-read signal — the file is written in all three modes, so never key on it existing or on the heading being present. The single-writer rule is **file-scoped**: `tech-plan` (including headless `actionReplan`), `implement`, `verify`, `next`, `review-plans` and `debug-manual` read it and never write it. `ux-design` is interactive-only and the auto loop never invokes it. Enforced mechanically by `runDesignAuthorityGuard`, which hashes `{base}/UX_DESIGN.md` and `.belmont/UX_DESIGN.md` around every agent shell-out in `executeLoopAction` and restores + steers on any change — needed because `copyBelmontStateToWorktree` and `syncFeatureStateAfterMerge` move the file in and out of worktrees outside git. Figma extraction deliberately **stayed** in `tech-plan`, so a Figma feature's tokens live in `{base}/TECH_PLAN.md` under `## Design Tokens (from Figma)` while its `**Mode**` lives in `UX_DESIGN.md`. **Known gap, not fixed here: `debug-auto` does not read the contract** — it carries zero contract references, and `_partials/debug-phase1-design.md` skips design analysis whenever there are no Figma URLs, which is exactly the UI-no-Figma case. An earlier version of this bullet claimed it did; that was false. Wiring it up is a behaviour change needing its own eval. See [`knowledge/cross-cutting/design-authority.md`](knowledge/cross-cutting/design-authority.md) and [`knowledge/cross-cutting/ux-design-phase.md`](knowledge/cross-cutting/ux-design-phase.md).
...
- `skills/belmont/` — Skill markdown files (product-plan, ux-design, tech-plan, implement, next, verify, status, reset). These are the source-of-truth copied/linked into target projects. The three planning skills run in that order: `product-plan` → `ux-design` → `tech-plan`, one artifact and one owner each — see [`knowledge/cross-cutting/ux-design-phase.md`](knowledge/cross-cutting/ux-design-phase.md).
