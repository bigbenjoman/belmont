...
- **A wave merge's state sync is scoped to its own milestone.** Wave merges are sequential into one main repo, so a whole-directory copy is last-writer-wins and silently reverts the marks earlier siblings earned (issue #24). `syncFeatureStateAfterMerge` takes a `milestoneID`: empty = whole-feature replace, non-empty = no deletes plus a `PROGRESS.md` splice of that milestone's block only. Do not widen it back. See [`knowledge/auto-mode/worktree-state-isolation.md`](knowledge/auto-mode/worktree-state-isolation.md).
