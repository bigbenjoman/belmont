...
- **Design authority is derived once, in `tech-plan`, and only that skill writes it.** A feature with a UI and no Figma gets a `## Design Contract` in `{base}/TECH_PLAN.md`, approved interactively. `implement`, `verify`, `next` and `debug-auto` read it and never write it; `review-plans` treats it as read-only; a headless `actionReplan` preserves it verbatim and never creates one. **This rule is prose-enforced and unguarded** — `runScopeGuard` parses `PROGRESS.md` only, and `syncFeatureStateAfterMerge` copies a worktree's feature dir over master's outside git — every file but `PROGRESS.md`, `TECH_PLAN.md` included — so a worktree phase that edits the section replaces the human-approved contract on main. A mechanical guard is deliberately deferred to its own proposal rather than smuggled onto the merge path. See [`knowledge/cross-cutting/design-authority.md`](knowledge/cross-cutting/design-authority.md).
