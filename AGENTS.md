...
- **Dependency eligibility has one definition.** `depSatisfied` (state.go) is what a satisfied `(depends: …)` means — the named milestone all-done, or naming none — consulted by `computeWaves`' in-degree count, `nextMilestone`, `nextTask` and `nextBlockedMilestone`, with the same rule stated in the selector prose of `implement`/`next`/`loop`/`status`. When everything pending is dependency-blocked, the views print `(waiting on dependencies) …`, **never** `None` — loop stop conditions read `None` as finished. See [`knowledge/cross-cutting/dependency-eligibility.md`](knowledge/cross-cutting/dependency-eligibility.md).
...
- **Auto refuses a dirty working tree; `update` auto-commits its own files; and a run may never dirty the tree it just checked.** `runAutoPreflight` blocks `belmont auto` startup when uncommitted/untracked changes exist (escape hatch: `--allow-dirty`), then guarantees `.belmont/metrics/` is gitignored — committing that one line when it had to write it, because `ensureStateFiles` writes the rule only at *install* time and every project installed before the metrics feature had none. Anything `auto` writes outside a worktree is committed by the run or ignored before the first write; the ordering (check first, guarantee second) is what keeps a failure from refusing a run that worked yesterday. `commitBelmontUpdate` stages and commits only Belmont-managed paths after `belmont update` (opt-out: `--no-commit`). See [`knowledge/auto-mode/clean-tree-preflight.md`](knowledge/auto-mode/clean-tree-preflight.md).
...
the first not-all-done milestone with its `(depends: …)` met and `implement.md`
picks the first eligible one holding any non-`[v]` task, so a `[!]` re-targets
the same milestone forever unless the caller names one explicitly. **Grinding at it** is worse: a human-gated follow-up
