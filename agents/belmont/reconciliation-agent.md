...
**Monorepo lock files.** In monorepos (`BELMONT_MONOREPO=1`), the lockfile lives at the repo root and is regenerated by running the workspace tool's install command at the root — `pnpm install` / `yarn install` / `npm install` / `cargo generate-lockfile`. This is correct: pnpm, npm-workspaces, yarn-workspaces, turbo, nx, and Cargo all install all workspace packages from one root invocation. Do NOT attempt per-workspace lockfile regeneration; there is no such thing in any of these systems.
